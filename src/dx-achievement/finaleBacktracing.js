    ...
    if (possibleValues.length === 3) {
        for (let i = noteCount; i >= 0; i--) {
            ...
            for (let j = noteCount - i; j >= jBound; j--) {
                ...
                const success = backtrace(judgements.slice(0, judgements.length - 1), remainingScore - possibleValues[1] * j - possibleValues[2] * k, dist, validDists);
                if (dist.get(2000) && dist.get(1250)) {
        ...
        backtrace(judgements.slice(0, judgements.length - 1), score - possibleValues[0] * noteCount, dist, validDists);
    }
    else {
        console.error('Unreachable');
    }
}
function calculateDxAchvFromFinaleResult(achv, totalScore, breakScore, breakJudgements) {
    const hundredAchvScore = (0, number_helper_1.roundFloat)((totalScore / achv) * 100, 'floor', 50);
    ...
    const breakDistributions = [];
    backtrace(breakJudgements, breakScore, new Map(), breakDistributions);
    if (!breakDistributions.length) {
        console.warn(`Could not find break distribution for achv=${achv}, totalScore=${totalScore}, breakScore=${breakScore}, breakJudgements=${breakJudgements.join('-')}`);
        return new Map();
    }
    ...
    const possibleDxAchvs = new Map();
    for (const dist of breakDistributions) {
        let achv = achvWithoutBreak;
        dist.forEach((count, score) => {
            achv += count * constants_1.BREAK_BASE_SCORE_MULTIPLIER.get(score) * baseAchvPerBreak;
            achv += count * constants_1.BREAK_BONUS_MULTIPLIER.get(score) * bonusAchvPerBreak;
        });
        const achvText = (0, number_helper_1.roundFloat)(achv, 'floor', 0.0001).toFixed(4);
        possibleDxAchvs.set(achvText, dist);
    }
    console.log(possibleDxAchvs);
    return possibleDxAchvs;
}
exports.calculateDxAchvFromFinaleResult = calculateDxAchvFromFinaleResult;
