export async function addPlayAndLastPlayedInfo(document: Document) {
  ...
  for (const block of blocks) {
    ...
    tasks.push(async () => {
      try {
        ...
        playCountDiv.className = 'music_score_block w_120 d_ib t_r f_12 play-count';
        playCountDiv.textContent = `🕹️ ${playCount} plays`;

        const lastPlayedDiv = document.createElement('div');
        lastPlayedDiv.className = 'music_score_block w_310 m_r_0 d_ib t_r f_12 last-played';
  ...
  // Throttle: run tasks in batches of 5 at a time
  const batchSize = 5;
  for (let i = 0; i < tasks.length; i += batchSize) {
    const batch = tasks.slice(i, i + batchSize);
    await Promise.allSettled(batch.map(fn => fn()));
    await new Promise((resolve) => setTimeout(resolve, 500)); // pause to avoid rate-limiting
  }
}
